...
namespace cAlgo.Robots
{
    ...
    public class ClaudeORB_v2 : Robot
    {
        ...
        // ── Paraméterek: Adaptív Rezsim ──────────────────────────
        [Parameter("Enable Fakeout Fader", Group = "Strategy", DefaultValue = true)]
        public bool EnableFakeoutFader { get; set; }

        [Parameter("Consecutive Losses to Switch", Group = "Strategy", DefaultValue = 2, MinValue = 1, MaxValue = 10)]
        public int ConLossesToSwitch { get; set; }

        [Parameter("Switch Back after Wins", Group = "Strategy", DefaultValue = 1, MinValue = 1, MaxValue = 10)]
        public int SwitchBackWins { get; set; }
        ...
        // ── Állapot: adaptív fázisváltó ──────────────────────────
        private bool _inFadeMode = false;
        private int _consecutiveLosses = 0;
        private int _fadeWins = 0;
        ...
        protected override void OnStart()
        {
            ...
            Log(LogVerbosity.Trades, "START",
                $"ORB-Prop v5 (v2 Adaptive) | sym={SymbolName} tf={TimeFrame} bal={Account.Balance:F2} " +
                $"pip={EffectivePip():F2}(sym={Symbol.PipSize}) autoSession={AutoUsSession} " +
                $"news={UseNewsFilter} rvol={UseRelVolFilter} fader={EnableFakeoutFader}(losses={ConLossesToSwitch}, back={SwitchBackWins})");
        }
        ...
        {
            TradeType breakoutDir = dir;
            bool fadeActive = EnableFakeoutFader && _inFadeMode;
            TradeType tradeDir = fadeActive ? (dir == TradeType.Buy ? TradeType.Sell : TradeType.Buy) : dir;

            double entry = tradeDir == TradeType.Buy ? Symbol.Ask : Symbol.Bid;
            
            double slDistance;
            if (!fadeActive)
            {
                double slPrice = tradeDir == TradeType.Buy ? _orLow : _orHigh;
                slDistance = Math.Abs(entry - slPrice);
            }
            else
            {
                double rangeWidth = _orHigh - _orLow;
                slDistance = rangeWidth + (tradeDir == TradeType.Buy ? (Symbol.Ask - Symbol.Bid) : 0);
            }
            ...
            var result = ExecuteMarketOrder(tradeDir, SymbolName, volume, Label, slPipsNative, tpPipsNative);
            if (result.IsSuccessful)
            {
                ...
                string modeStr = fadeActive ? "FADE" : "BREAKOUT";
                Log(LogVerbosity.Trades, "ENTRY",
                    $"{modeStr} {tradeDir} (Breakout: {breakoutDir}) @ {_entryPrice} | SL={slPipsDisplay:F1}p TP={slPipsDisplay * TpRMultiple:F1}p ({TpRMultiple:F1}R) " +
                    $"vol={volume} risk={riskMoney:F2} ({RiskPercent}%) spread={Symbol.Spread / EffectivePip():F1}p");
            }
            else
            {
                Log(LogVerbosity.Trades, "ORDER-FAIL", $"{tradeDir} sikertelen: {result.Error}");
            }
        }
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (EnableFakeoutFader)
            {
                bool wasFade = _inFadeMode;
                bool isWin = p.GrossProfit > 0;
                ...
                if (!wasFade)
                {
                    ...
                    {
                        _consecutiveLosses++;
                        if (_consecutiveLosses >= ConLossesToSwitch)
                        {
                            _inFadeMode = true;
                            _fadeWins = 0;
                            Log(LogVerbosity.Trades, "REGIME", $"Váltás FADE MÓDRA | Sorozatos vesztő = {_consecutiveLosses}");
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        _fadeWins++;
                        if (_fadeWins >= SwitchBackWins)
                        {
                            _inFadeMode = false;
                            _consecutiveLosses = 0;
                            Log(LogVerbosity.Trades, "REGIME", $"Visszaváltás BREAKOUT MÓDRA | Sikeres fade trade-ek = {_fadeWins}");
                        }
                    }
                    else
                    {
                        _inFadeMode = false;
                        _consecutiveLosses = 0;
                        Log(LogVerbosity.Trades, "REGIME", $"Failsafe visszaváltás BREAKOUT MÓDRA | Fade trade elbukott (valódi kitörés)");
                    }
                }
            }
