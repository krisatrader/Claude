"""
NAS100 ORB Regime Engine v5b — M15 60-day NQ=F
Csak [B] módosítás v4-hez képest (v5-ből [C] eltávolítva):

  [B] Module B ADX cap: max ADX_sm=32 — megakadályozza a belépést magas-ADX,
      irányváltó piacokon. Megtartott: OR-alapú rezsim-detektálás.

  [C] ELTÁVOLÍTVA: bar strength fix 0.60, nincs adaptív enyhítés.
      Ok: v5-ben a relaxált küszöb (ADX_sm>35 → 0.50) beengedte a 04-17
      veszteséges belépést (bar_str=0.54), haszon nélkül.

Cél: v4 ápr/máj megtartása + júniusi rés csökkentése
"""
...
# Module A — ORB Trend (unchanged from v4, [C] removed)
RISK_BASE_A        = 0.010
RISK_GRADE_A       = 0.015
ADX_GRADE_A        = 28.0
CI_GRADE_A         = 45.0
...
PARTIAL_R          = 1.5
TRAIL_MULT         = 2.0
BAR_STRENGTH       = 0.60       # fix 0.60, nincs adaptív enyhítés
ORB_QUALITY        = 0.35

# Module B — BB Reversion ([B] ADX cap)
RISK_B             = 0.005
ATR_SL_B           = 1.2
RSI_LONG           = 25
RSI_SHORT          = 75
...
BB_PERIOD          = 20
BB_STD             = 2.0
MAX_MODB_ADX_SM    = 32.0       # [B] Module B max ADX_sm plafon
...
def close_t(t, ep, ts, reason, partial=False, pvol=None):
    ...
    log.append({
        ...
        'pdi_ndi_sm': t.get('pdi_ndi_sm', 0), 'bar_str': t.get('bar_str', 0),
    })
    if partial:
        t['vol'] -= vol
    else:
...
print("Running v5b backtest (only [B] ADX cap, no [C])...")
b_blocked_adx = 0
...
for ts, row in raw.iterrows():
    ...
    if regime == 'trend' and orb_done and not ny_traded and (15, 0) <= hm < (21, 0):
        ...
        if orb_r >= av * ORB_QUALITY:
            ...
            # [C] ELTÁVOLÍTVA: csak fix BAR_STRENGTH = 0.60
            if p > orb_hi + SPREAD_PTS and p > ev and pv > nv and bsp >= BAR_STRENGTH:
                ...
                trade = {
                    ...
                    'adx_sm': as_, 'ci_sm': cs_, 'pdi_ndi_sm': ps_, 'bar_str': bsp,
                }
                ny_traded = True
            elif p < orb_lo - SPREAD_PTS and p < ev and nv > pv and bsp <= (1.0 - BAR_STRENGTH):
                rk = RISK_GRADE_A if dv > ADX_GRADE_A and cv < CI_GRADE_A else RISK_BASE_A
                sd = av * ATR_SL_A; rpc = sd * POINT_VALUE
                v = max(1, int(balance * rk / rpc)) if rpc > 0 else 1
                trade = {
    ...
    # ── MODULE B — BB Reversion entry ─────────────────────────────────────────
        ...
        # [B] ADX cap: nem lép be ha ADX_sm > MAX_MODB_ADX_SM
        if as_ >= MAX_MODB_ADX_SM:
            ...
            if p < bl and rv < RSI_LONG and (bm - p) > sd and p > ev:
                trade = {
                    ...
                    'vol': v, 'vf': v, 'entry_time': ts, 'be': False, 'pd': False,
                    'ts2': None, 'pts_risk': sd, 'regime': regime,
                    'adx_sm': as_, 'ci_sm': cs_, 'pdi_ndi_sm': ps_, 'bar_str': 0,
                }
                range_td += 1
...
print()
print("=" * 70)
print("NAS100 ORB REGIME ENGINE v5b — M15 BACKTEST  (csak [B], [C] nélkül)")
print(f"[B] Module B ADX cap = {MAX_MODB_ADX_SM:.0f}   |   [C] Bar strength: fix {BAR_STRENGTH}")
print("=" * 70)
...
print()
print(f"[B] ADX cap blokkolta Module B-t: {b_blocked_adx}× alkalommal (ADX_sm ≥ {MAX_MODB_ADX_SM:.0f})")
print()
print("Havi bontás:")
...
print(f"  v3:  Net +$20,956 | Ápr +$2,654  | Máj  -$5,940 | Jún +$24,242 | DD 14.1%")
print(f"  v4:  Net +$18,246 | Ápr +$5,517  | Máj  +$2,906 | Jún  +$9,823 | DD  5.8%")
print(f"  v5:  Net +$19,605 | Ápr +$2,732  | Máj    +$943 | Jún +$15,930 | DD  7.3%")
print(f"  v5b: Net ${net:+,.0f} | ", end="")
for m, mp in monthly.items():
...
disp = df_log[['entry_time', 'exit_time', 'module', 'dir', 'entry', 'exit',
               'pnl', 'reason', 'regime', 'adx_sm', 'ci_sm', 'pdi_ndi_sm']].copy()
