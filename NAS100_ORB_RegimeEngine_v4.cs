/// <summary>
/// NAS100 ORB + Regime Engine cBot  — v4.0
/// =========================================
...
///   2. Module A — ORB Trend  — EMA200 + PDI/NDI + ORB breakout + bar-strength filter
///                               Csak Trend rezsimben aktív (simított ADX>25, CI<52, |PDI-NDI|>10)
///   3. Module B — BB Reversion — Bollinger Bands(20,2) + RSI(7) + EMA200 irányfilter
///                               Csak Range rezsimben aktív (simított ADX<22, CI>57, |PDI-NDI|<6)
...
///
/// v4.0 változások (Python backtest validált: Apr+5.5%, Máj+2.9%, Jún+9.8%, FTMO DD 0%):
///   [NEW]    Smoothed Regime: ADX/CI/PDI-NDI 16-bar EMA simítás — stabilabb rezsim-váltás
...
///   [REMOVE] VWAP Range Engine (v3 Range Engine lecserélve Module B-re)
///   [FIX]    ManageOpenPositions: külön exit logika Module A és B számára
///
...
namespace cAlgo.Robots
{
    ...
    public class NAS100_ORB_RegimeEngine_v4 : Robot
    {
        ...
        protected override void OnStart()
        {
            ...
            Print($"[v4] ORB: {OrbStartHour}:{OrbStartMinute:D2}+{OrbRangeMinutes}min | " +
                  $"Smooth={SmoothedPeriod} | Trend ADX>{TrendAdxMinSm} CI<{TrendCiMaxSm} |PDI-NDI|>{TrendPdiNdiMinSm}");
            Print($"[v4] Range ADX<{RangeAdxMaxSm} CI>{RangeCiMinSm} |PDI-NDI|<{RangePdiNdiMaxSm} | ModB risk={RangeRiskPercent}% SL=ATR×{RangeSlAtrMult}");
        ...
        {
            Print(Sep);
            Print($"[LOGGER] NAS100 ORB + Regime Engine — v4.0 — INDULÁS");
